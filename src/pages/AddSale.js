...
//               </li>
//               <li>
//                 <Link className="btn btn-primary" to="/sale-list">
...
import { useState } from "react"
import { connectWallet, startTokenSale } from "../lib/contract"
import Swal from "sweetalert2"
import { Link } from "react-router-dom"
...
const AddSale = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const saleTypeMap = { Private: 0, Public: 1 }
      const processedFormData = {
        ...formData,
        saleType: saleTypeMap[formData.saleType],
      }

      console.log("[v0] Processed form data:", processedFormData)

      const result = await startTokenSale(processedFormData)
      ...
      Swal.fire({
        ...
        html: `
          <div class="text-start">
            <p><strong>Transaction Hash:</strong><br><code>${result.transactionHash}</code></p>
            <p><strong>Sale Type:</strong> ${formData.saleType}</p>
            <p><strong>Token Price:</strong> ${formData.tokenPrice} BNB</p>
            <p><strong>Total Tokens:</strong> ${formData.totalTokens}</p>
            ${result.saleId ? `<p><strong>Sale ID:</strong> ${result.saleId}</p>` : ""}
          </div>
        `,
        confirmButtonText: "View on BSCScan",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
          window.open(result.value || `https://testnet.bscscan.com/tx/${result.transactionHash}`, "_blank")
        }
      })
